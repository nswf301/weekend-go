...
- 제목을 **"스마트 퀴즈존"**으로 바꿈(옛 이름 "즐거운 QR게임"). `config.js`·`index.html`·관리 화면.
- 숫자 누르기 1판 결과 문구: "다음 판은 **10초 안에** 눌러 주세요".
- 결과 배지를 문장 대신 낱말로: 잘하셨습니다 → **성공**, 정답입니다 → **정답**. "아쉬워요"는 그대로.
- 배지는 한 줄을 따로 쓰고 설명문은 그 아래 줄에서 시작(`style.css` `.badge` display:block).
  한 줄이 늘어 팝업 고정 높이를 다시 잼(360px 폭): 일반 330 → **340**(최대 339.6), 순서 기억하기 344 → **388**(최대 387.4).
